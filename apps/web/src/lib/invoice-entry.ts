...
/**
 * Manual invoice entry: the rules behind /invoices/new.
 *
 * POST /invoices has always existed (the AI-off path); nothing called it. The
 * page is thin and everything that decides what is sent lives here, so it can
 * be proven without a browser. apps/mobile/src/lib/invoice-entry.ts mirrors it
...
/** A calendar date as the API takes it; 2026-02-31 is not one. */
...
/**
 * A decimal-string amount for display. INR goes through formatInr so the
 * grouping is the Indian one used across the web app (1,08,000.00); other
 * currencies keep their code and Western grouping. String-based for those, so
 * a large amount is never rounded through a float.
 */
export function formatInvoiceMoney(amount: string | null | undefined, currency: string): string {
  if (amount == null || amount === '') return '—';
  if (currency === 'INR') return formatInr(Number(amount), { paise: true });
